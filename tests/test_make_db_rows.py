...
def _make_sheet(path: Path) -> None:
    ...
    ws.append(SHEET_SUB)
    ws.append([1, "Nb01", None, "TargetA", "recb", "P", "SetA", 20, P1,
               "AAA", "DEF", "GHIK", "P4102_95", None, None, None, 1, None, None])
    ws.append([2, "Nb02", None, "TargetA", "recb", "Y", "SetB", 20, P2,
...
def run_dir(tmp_path: Path) -> Path:
    _make_sheet(tmp_path / "plasmid_database.xlsx")
...
def test_sequences_and_inherited_metadata_come_from_the_run(run_dir: Path) -> None:
    ...
    assert by_id["Nb03"]["Target"] == "TargetA"
    assert by_id["Nb03"]["Source"] == "P"
    assert by_id["Nb03"]["Set name"] == "SetA"
    assert by_id["Nb05"]["Source"] == "Y"
    assert by_id["Nb05"]["Set name"] == "SetB"
